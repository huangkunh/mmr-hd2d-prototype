...
func _setup_sprite() -> void:
        ...
        # Add a soft shadow blob beneath the sprite for grounding (HD-2D touch)
        _setup_shadow_blob()


func _setup_shadow_blob() -> void:
        var shadow := get_node_or_null("ShadowBlob")
        if shadow:
                return
        var blob := Sprite3D.new()
        blob.name = "ShadowBlob"
        blob.pixel_size = 0.015625
        blob.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
        blob.no_depth_test = true
        blob.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
        blob.rotation_degrees.x = -90.0  # lay flat on ground
        blob.position = Vector3(0, 0.02, 0)
        # Procedural radial-gradient shadow texture
        ...
        for y in range(16):
                for x in range(32):
                        ...
                        var dy := (float(y) - cy) / 8.0
                        var d := sqrt(dx * dx + dy * dy)
                        if d < 1.0:
                                var a: float = clamp(1.0 - d, 0.0, 1.0) * 0.55
                                img.set_pixel(x, y, Color(0, 0, 0, a))
        blob.texture = ImageTexture.create_from_image(img)
        blob.modulate = Color(0, 0, 0, 0.5)
        add_child(blob)


func _create_placeholder_frames() -> void:
        # Always create fresh placeholder frames (procedural pixel-art hunter)
...
func _make_placeholder_texture(color: Color) -> ImageTexture:
        # A more detailed 16x24 pixel-art hunter: hat, head, jacket, belt, pants, boots
        var img := Image.create(16, 24, false, Image.FORMAT_RGBA8)
        img.fill(Color(0, 0, 0, 0))  # Transparent
        # Palette
        var skin := Color(0.85, 0.65, 0.45, 1.0)
        var hat := Color(0.18, 0.14, 0.10, 1.0)
        var hat_hi := Color(0.30, 0.24, 0.18, 1.0)
        var jacket := color
        var jacket_hi := color.lightened(0.18)
        var belt := Color(0.20, 0.15, 0.10, 1.0)
        var pants := Color(0.25, 0.28, 0.35, 1.0)
        var boots := Color(0.12, 0.10, 0.08, 1.0)
        var outline := Color(0.05, 0.04, 0.03, 1.0)
        for y in range(24):
                for x in range(16):
                        var px := false
                        var c: Color = color
                        # Hat brim (row 0-1)
                        if y <= 1 and x >= 3 and x <= 12:
                                px = true; c = hat
                        # Hat crown (rows 2-4)
                        elif y >= 2 and y <= 4 and x >= 5 and x <= 10:
                                px = true; c = hat if x % 2 == 0 or y == 2 else hat_hi
                        # Face (rows 5-8)
                        elif y >= 5 and y <= 8 and x >= 5 and x <= 10:
                                px = true; c = skin
                        # Neck/jacket collar (row 9)
                        elif y == 9 and x >= 5 and x <= 10:
                                px = true; c = jacket_hi
                        # Torso / jacket (rows 10-16)
                        elif y >= 10 and y <= 16 and x >= 4 and x <= 11:
                                px = true
                                c = jacket_hi if (x == 4 or x == 11 or y == 10) else jacket
                        # Belt (row 17)
                        elif y == 17 and x >= 4 and x <= 11:
                                px = true; c = belt
                        # Pants (rows 18-21)
                        elif y >= 18 and y <= 21 and ((x >= 4 and x <= 7) or (x >= 8 and x <= 11)):
                                px = true; c = pants
                        # Boots (rows 22-23)
                        elif y >= 22 and ((x >= 4 and x <= 7) or (x >= 8 and x <= 11)):
                                px = true; c = boots
                        if px:
                                # Add a darker outline on left edge for depth
                                img.set_pixel(x, y, c)
...
func _play_walk_animation() -> void:
        var anim: String = DIR_FRAMES.get(_facing, "walk_down")
        if _sprite.sprite_frames and _sprite.sprite_frames.has_animation(anim) and _sprite.animation != anim:
                _sprite.play(anim)


func _play_idle_animation() -> void:
        if velocity.length() < 0.5:
                var anim: String = IDLE_FRAMES.get(_facing, "idle_down")
                if _sprite.sprite_frames and _sprite.sprite_frames.has_animation(anim) and _sprite.animation != anim:
                        _sprite.play(anim)
